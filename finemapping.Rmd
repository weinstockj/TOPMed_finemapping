...
params:
    ...
    ABC_cell_type: "CD34-positive_mobilized-Roadmap"
    ABC_gene: "SMC4"
output: 
...
ABC_cell_type = params$ABC_cell_type
ABC_gene = params$ABC_gene
...
ABC = data.table::fread(cmd = ABC_cmd) %>%
    ...
    dplyr::filter(CellType == .env[["ABC_cell_type"]]) %>%
    dplyr::select(
        CHROM,
        start = START,
        end = END,
...
locuszoom = function(variant_meta, susie_model, ld2, snp_to_gene, ABC) {
    ...
    if(!is.null(ABC)) {
        p1 = p1 +
            geom_rect(
                data = ABC %>% dplyr::filter(TargetGene == .env[["ABC_gene"]]),
...
```{r read_summary}
cmd = glue::glue("tabix {summary_stats_file} {region}")
...
if(!is.null(rss$sets)) {
    ...
    for (l in 1:L) {
        ...
        causal_snps = summary_stats %>%
            ...
            plyranges::join_overlap_left(plyranges::as_granges(ABC)) %>%
